...
#   ./build_cpanel.sh --publish  rebuild, then copy into ~/Desktop/CPANEL/mysteryshop
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="$HERE/flask"
DST="$HERE/cpanel"
PUBLISH_DIR="$HOME/Desktop/CPANEL/mysteryshop"
...
if [ "${1:-}" = "--publish" ]; then
  ...
  echo "    now: ~/Desktop/CPANEL_PUSH.sh check mysteryshop"
fi
